...
<body>
<main class="wrap">
<h1>Cosmo Sofa Admin</h1>
<div class="muted">Тестирование первичного onboarding. История публикаций при сбросе сохраняется.</div>
<div class="auth"><input id="token" type="password" placeholder="ADMIN_TOKEN"><button id="load">Показать пользователей</button></div>
...
async function api(path,opts={}){const t=token.value.trim();sessionStorage.setItem('adminToken',t);const r=await fetch(path,{...opts,headers:{...(opts.headers||{}),Authorization:'Bearer '+t}});const d=await r.json();if(!r.ok)throw Error(d?.error?.message||'Ошибка запроса');return d;}
function userCard(u){
  const title=esc(u.bot_name||u.telegram_user_id||u.id);
  const id=esc(u.id);
  const created=esc(u.created_at);
  const telegramId=esc(u.telegram_user_id||'—');
  const bot=esc(u.bot_username?'@'+u.bot_username:'—');
  const tgGroup=esc(u.telegram_group||'—');
  const vk=esc(u.vk_group_name||u.vk_group_url||'—');
  const history=esc(u.post_count||0);
  return '<section class="card row"><div><div class="name">'+title+'</div><div class="meta">'+id+'<br>Создан: '+created+'</div></div><div class="chips"><span class="chip">Telegram ID: '+telegramId+'</span><span class="chip">Бот: '+bot+'</span><span class="chip">TG группа: '+tgGroup+'</span><span class="chip">VK: '+vk+'</span><span class="chip">История: '+history+'</span></div><button class="danger" data-reset="'+id+'">Сбросить onboarding</button></section>';
}
async function load(){status.textContent='Загрузка…';users.innerHTML='';try{const d=await api('/api/admin/users');status.textContent='Пользователей: '+d.users.length;users.innerHTML=d.users.length?d.users.map(userCard).join(''):'<div class="card empty">Пользователей нет</div>';}catch(e){status.textContent=e instanceof Error?e.message:String(e);}}
users.addEventListener('click',async(e)=>{const b=e.target.closest('[data-reset]');if(!b)return;const id=b.dataset.reset;if(!confirm('Сбросить onboarding этого пользователя?\n\nПерсональный бот, Telegram-группа и VK-группа будут отвязаны. История публикаций сохранится.'))return;b.disabled=true;try{await api('/api/admin/users/'+encodeURIComponent(id)+'/reset-onboarding',{method:'POST'});await load();}catch(x){alert(x instanceof Error?x.message:String(x));b.disabled=false;}});
