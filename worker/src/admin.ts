  ...
  const [usersResult, botsResult] = await Promise.all([
    env.DB.prepare(`
      SELECT u.id,u.google_sub,u.created_at,u.updated_at,
        ...
        (SELECT ti.first_name FROM telegram_identities ti WHERE ti.user_id=u.id LIMIT 1) AS telegram_first_name,
        (SELECT ti.last_name FROM telegram_identities ti WHERE ti.user_id=u.id LIMIT 1) AS telegram_last_name,
        (SELECT ti.username FROM telegram_identities ti WHERE ti.user_id=u.id LIMIT 1) AS telegram_username,
        (SELECT ti.photo_url FROM telegram_identities ti WHERE ti.user_id=u.id LIMIT 1) AS telegram_photo_url,
...
function card(u){const id=esc(u.id),bots=u.telegram_bots||[],vk=!!u.vk_group_id,name=[u.telegram_first_name,u.telegram_last_name].filter(Boolean).join(' ').trim(),title=name?esc(name):id,avatar=u.telegram_photo_url?'<img class="avatar" src="'+esc(u.telegram_photo_url)+'" alt="">':'',meta=[name?'User ID: '+id:null,'Telegram user: '+esc(u.telegram_user_id||'—'),u.telegram_username?'@'+esc(u.telegram_username):null,'Создан: '+esc(date(u.created_at))].filter(Boolean).join(' · ');return '<section class="user" data-user-id="'+id+'"><div class="user-head">'+avatar+'<div class="user-head-main"><div class="uid">'+title+'</div><div class="muted small">'+meta+'</div></div></div><div class="content"><div><b>Telegram</b><div class="muted small">Ботов: '+bots.length+'</div></div><div class="telegram">'+(bots.length?bots.map(botCard).join(''):'<div class="empty muted">Telegram-ботов нет</div>')+'</div><div class="vk"><div><div class="title-row"><span class="title">VK Group</span>'+state(vk)+'</div>'+(vk?'<div class="details">'+field('Title',u.vk_group_name)+field('Group ID',u.vk_group_id)+field('URL',u.vk_group_url)+field('Дата записи',date(u.vk_group_created_at))+'</div>':'<div class="muted">VK-группа не подключена</div>')+'</div><button class="danger" data-action="vk-group" '+(!vk?'disabled':'')+'>Удалить VK-группу</button></div></div><div class="danger-zone"><div><b>Удаление пользователя</b><div class="muted small">Удаляет пользователя, все его Telegram-боты и группы, VK и историю.</div></div><button class="ghost-danger" data-action="user">Удалить пользователя целиком</button></div></section>'}
