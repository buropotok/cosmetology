...
async function route(req: Request, env: Env) {
  ...
  /*
   * TEMPORARY TELEGRAM WEBHOOK SETUP ENDPOINT
   *
   * This endpoint is only needed to register the Telegram webhook
   * from Cloudflare because api.telegram.org is not reachable from
   * the local development machine.
   *
   * IMPORTANT:
   * Remove this entire route after the webhook has been successfully
   * registered.
   */
  if (
    req.method === 'POST' &&
    u.pathname === '/api/setup/telegram-webhook'
  ) {
    ...
    const webhookUrl = `${u.origin}/api/telegram/webhook`;
  ...
  /*
   * Telegram webhook itself remains permanent.
   */
  if (
    req.method === 'POST' &&
