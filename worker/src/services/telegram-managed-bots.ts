...
export async function getManagedBotCredential(env: Env, managedBotId: string) {
  const row = await env.DB.prepare(
    `SELECT telegram_bot_id,username,status,token_ciphertext,token_iv,token_key_version
     FROM telegram_managed_bots WHERE telegram_bot_id=?`,
  )
    .bind(managedBotId)
