...
export async function getMediaGallery(request:Request,env:Env){
  ...
  return {assets:assets.map(asset=>({id:asset.id,thumbnailId:asset.thumbnailId,fileName:asset.fileName,contentType:asset.contentType,size:asset.size,sourceType:asset.sourceType,createdAt:asset.createdAt,originalUrl:`/api/miniapp/media/original/${encodeURIComponent(asset.id)}`,thumbnailUrl:asset.thumbnailId?`/api/miniapp/media/thumbnail/${encodeURIComponent(asset.thumbnailId)}`:null}))};
}
