...
async function callWithToken(token:string,method:string,body?:FormData){
  ...
  if(!response.ok||!json?.ok){
    ...
    console.error({event:'telegram_bot_api_error',method,errorCode:json?.error_code??null,description});
...
export async function setTelegramWebhookWithToken(token:string,webhookUrl:string,secretToken:string){const body=new FormData();body.set('url',webhookUrl);body.set('secret_token',secretToken);body.set('allowed_updates',JSON.stringify(['message','my_chat_member']));return callWithToken(token,'setWebhook',body) as Promise<boolean>}
...
async function publishWithToken(token:string,rendered:TelegramRender|string,image:File|undefined,chatId:string){const rich=typeof rendered!=='string',value=rich?rendered as TelegramRender:{html:rendered as string,plainText:rendered as string,blocks:[],buttons:[]},keyboard=value.buttons?.length?JSON.stringify({inline_keyboard:value.buttons.map(button=>[{text:button.text,url:button.url}])}):undefined,plan=planTelegramPublication(value,!!image),{html,plainText}=value;if(plainText.length>4096)throw new AppError('TELEGRAM_TEXT_TOO_LONG','Telegram: текст не должен превышать 4096 символов',400);let first;if(image){const f=new FormData();f.set('chat_id',chatId);f.set('photo',image,'image');if(plan.type==='photo_with_caption'){f.set('caption',html);if(rich)f.set('parse_mode','HTML');if(keyboard)f.set('reply_markup',keyboard)}first=await callWithToken(token,'sendPhoto',f);if(plan.type==='photo_then_text'){const t=new FormData();t.set('chat_id',chatId);t.set('text',html);if(rich)t.set('parse_mode','HTML');if(keyboard)t.set('reply_markup',keyboard);await callWithToken(token,'sendMessage',t)}}else{const f=new FormData();f.set('chat_id',chatId);f.set('text',html);if(rich)f.set('parse_mode','HTML');if(keyboard)f.set('reply_markup',keyboard);first=await callWithToken(token,'sendMessage',f)}return {external_id:String(first.message_id),url:url(chatId,first.message_id),delivery_mode:plan.type}}
export async function publishTelegram(env:Env,rendered:TelegramRender|string,image:File|undefined,chatId:string){return publishWithToken(env.TELEGRAM_BOT_TOKEN,rendered,image,chatId)}
export async function publishTelegramWithToken(token:string,rendered:TelegramRender|string,image:File|undefined,chatId:string){return publishWithToken(token,rendered,image,chatId)}
