...
import { createManagedBotLink, getManagerBotDiagnostic, handleManagedBotUpdate, type ManagedBotTelegramApi } from './telegram-managed-bots';
...
describe('Telegram Managed Bots PoC', () => {
  ...
  it('recognizes ManagedBotUpdated, fetches its token and verifies it with getMe', async () => { const telegram = api(); const { env, writes } = testEnv(); const log = vi.spyOn(console, 'log').mockImplementation(() => undefined); await expect(handleManagedBotUpdate(managedUpdate, env, telegram)).resolves.toBe(true); expect(telegram.getManagedToken).toHaveBeenCalledWith(env, '9001'); expect(telegram.getManagedMe).toHaveBeenCalledWith(managedToken); expect(writes[0].values).toContain('9001'); expect(log).toHaveBeenCalledWith({ event: 'telegram_managed_bot_token_verified', managedBotId: '9001', managedBotUsername: 'created_bot', verified: true }); const serialized = JSON.stringify({ logs: log.mock.calls, writes }); expect(serialized).not.toContain(token); expect(serialized).not.toContain(managedToken); });
