...
async function contentHash(image: File) { return toHex(new Uint8Array(await crypto.subtle.digest('SHA-256', await image.arrayBuffer()))); }
...
export async function storePermanentMediaAsset(env: Env,userId:string,image:File,sourceType:string) {
  ...
  const existing=await env.DB.prepare('SELECT id,r2_key AS key,file_name AS fileName,content_type AS contentType,size_bytes AS size,source_type AS sourceType,content_hash AS contentHash,created_at AS createdAt FROM media_assets WHERE user_id=? AND content_hash=? LIMIT 1').bind(userId,hash).first<MediaAsset>();
  if(existing)return {asset:existing,created:false};
  const id=crypto.randomUUID(),key=`media/${userId}/${id}`;
  await env.IMAGES.put(key,image.stream(),{httpMetadata:{contentType:image.type}});
  await env.DB.prepare('INSERT INTO media_assets(id,user_id,r2_key,source_type,file_name,content_type,size_bytes,content_hash) VALUES(?,?,?,?,?,?,?,?)').bind(id,userId,key,sourceType,image.name||null,image.type||null,image.size,hash).run();
  return {asset:{id,key,fileName:image.name||null,contentType:image.type||null,size:image.size,sourceType,contentHash:hash,createdAt:new Date().toISOString()},created:true};
}
