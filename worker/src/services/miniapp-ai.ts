...
async function callOpenAI(
  ...
  input: string,
  signal: AbortSignal,
  options: { webSearch?: boolean; instructions?: string } = {},
) {
  ...
  const text = extractOpenAIText(payload, options.webSearch);
...
function parseDiscovery(text: string) {
  ...
  if (value.schemaVersion !== 1 || !Array.isArray(value.ideas) || value.ideas.length !== 5) {
  ...
  value.ideas.forEach((raw, index) => {
    ...
    if (
      ...
      throw new Error('Invalid discovery idea');
    }
    ids.add(expectedId);
    if (
      idea.date !== undefined &&
      (typeof idea.date !== 'string' || !/^\d{4}-\d{2}-\d{2}$/.test(idea.date))
    ) {
...
export async function generateMiniAppAiReply(req: Request, env: Env) {
  ...
  try {
    ...
    await setAiGenerationStatus(
      env,
      userId,
      kind,
      'failed',
      cancelled ? 'AI_GENERATION_CANCELLED' : 'AI_GENERATION_FAILED',
    ).catch(statusError =>
      console.error('Failed to persist AI generation failure', statusError),
    );
    ...
    throw new AppError(
      ...
      'Не удалось получить ответ AI. Попробуйте ещё раз.',
      502,
    );
  }
}
