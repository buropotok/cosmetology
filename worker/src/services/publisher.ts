...
export async function publishPlatform(env: Env, platform: Platform, text: string, image?: File, document?: PostDocument) {
  if (platform === 'vk') {
  ...
  return publishTelegram(env, document ? renderTelegram(document) : text, image);
}
