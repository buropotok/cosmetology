...
describe('Mini App bootstrap',()=>{
  ...
  it('preserves first-party startup order inside bootstrap',()=>{const bootstrap=miniappFile('bootstrap.js');const ordered=['telegram-gateway.js','app-router.js','app.js','navigation.js','account-state.js','onboarding-api.js','onboarding-controller.js','onboarding-view.js','onboarding-router.js','settings.js','composer-mockup.js'];for(let i=1;i<ordered.length;i++)expect(bootstrap.indexOf(ordered[i-1])).toBeLessThan(bootstrap.indexOf(ordered[i]))});
