...
describe('Mini App bootstrap',()=>{
  it('is the only loader for first-party startup modules',()=>{
    ...
    expect(scriptSources).toContain('/bootstrap.js');expect(scriptSources).toHaveLength(2);expect(scriptSources).toContain('/vendor/telegram-web-app.js');expect(scriptSources).not.toContain('https://telegram.org/js/telegram-web-app.js');expect(miniappFile('composer-mockup.js')).not.toContain("import('/navigation.js')");expect(miniappFile('drafts.js')).not.toContain("import('/navigation.js')");
  });
  ...
  it('defers production Tiptap runtime to the New Post lifecycle',()=>{
    ...
    expect(navigation).not.toContain('ai-test-input');
    expect(navigation).not.toContain("'/api/miniapp/ai/chat'");
    expect(navigation).not.toContain("#flow-edit");
  });
  it('lets navigation own the Home settings button and uses the shared file asset',()=>{const navigation=miniappFile('navigation.js'),html=miniappFile('index.html');expect(navigation).toContain("background:transparent url('/assets/icons/settings.svg') center/24px 24px no-repeat");expect(navigation.match(/class=\"cosmo-flow-settings cosmo-settings-button\"/g)).toHaveLength(1);expect(navigation).not.toContain("querySelector('#open-settings')");expect(html).not.toContain('id="open-settings"');expect(html).not.toContain('M19.14 12.94');expect(miniappFile('assets/icons/settings.svg')).toContain('M19.14 12.94')});
  it('loads the Telegram capability gate after composer actions without persisted reconciliation',()=>{const bootstrap=miniappFile('bootstrap.js'),flow=miniappFile('onboarding-flow.js');expect(bootstrap.indexOf("'/composer-actions.js'")).toBeLessThan(bootstrap.indexOf("'/onboarding-flow.js'"));expect(flow).toContain('window.CosmoAccountState');expect(flow).toContain('window.CosmoOnboardingFlow=Object.freeze({guard})');for(const obsolete of ["'/api/miniapp/onboarding-intent'","'/api/miniapp/onboarding-flow'",'showPublishConfirmation','Диагностика onboarding flow','cosmo-onboarding-result'])expect(flow).not.toContain(obsolete)});
  it('keeps Telegram capability guards independent from onboarding run cancellation',()=>{const flow=miniappFile('onboarding-flow.js'),controller=miniappFile('onboarding-controller.js');expect(controller).toContain("this.finish('cancelled','user_back')");expect(flow).not.toContain("result?.status==='cancelled'&&result?.reason==='user_back'");expect(flow).not.toContain('Onboarding intent cancellation failed');expect(flow).not.toContain('reconcile(')});
  it('prepares VK, persists publish intent, and routes VPN fallback through the managed bot',()=>{
    ...
    expect(app).not.toContain('resumeVkPublishIntent');expect(app).not.toContain("window.addEventListener('focus'");
