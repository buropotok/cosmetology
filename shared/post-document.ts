...
export type PostBlock =
  | PostDetailsBlock
  | {type: 'details'; title?: TextRun[]; blocks: PostDetailsBlock[]; emoji?: string};
...
export function isPostDocument(value: unknown): value is PostDocument {
  ...
  const detailsBlock=(block:unknown):boolean=>{if(!block||typeof block!=='object'||!('type'in block))return false;const b=block as PostDetailsBlock;if('items'in b)return (b.type==='bullet_list'||b.type==='ordered_list')&&Array.isArray(b.items)&&b.items.every(item=>listItem(item));return ['paragraph','heading','quote'].includes(b.type)&&runs(b.content)};
  return document.blocks.every(block => {
    if (!block || typeof block !== 'object' || !('type' in block)) return false;
    const b = block as PostBlock;
    if(b.type==='details')return (b.title===undefined||runs(b.title))&&Array.isArray(b.blocks)&&b.blocks.every(detailsBlock);
    return detailsBlock(b);
  });
}
...
const blockText=(block:PostDetailsBlock):string=>'items'in block?block.items.map(itemText).join('\n'):block.content.map(run=>run.text).join('');
export function documentText(document: PostDocument): string {
  return document.blocks.map(block => block.type==='details'?block.blocks.map(blockText).join('\n'):blockText(block)).join('\n');
}
...
const blockHasSpoiler=(block:PostDetailsBlock):boolean=>'items'in block?block.items.some(itemHasSpoiler):block.content.some(run=>run.marks?.some(mark=>mark.type==='spoiler'));
export function hasTelegramSpecificFormatting(document: PostDocument) {
  return document.blocks.some(block => block.type === 'details' || blockHasSpoiler(block));
}
