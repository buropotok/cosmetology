...
export function isPostDocument(value: unknown): value is PostDocument {
  ...
  return document.blocks.every(block => {
    ...
    return 'items' in b ? (b.type==='bullet_list'||b.type==='ordered_list')&&Array.isArray(b.items) && b.items.every(runs) : ['paragraph','heading','quote','details'].includes(b.type) && runs(b.content)&&(b.type!=='details'||b.title===undefined||runs(b.title));
  });
}
