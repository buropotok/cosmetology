...
export function renderTelegram(document:PostDocument):TelegramRender{
  ...
  for(const block of document.blocks){
    ...
    else blocks.push({kind:block.type==='quote'?'quote':block.type==='details'?'expandable_quote':'text',source:block.type,segments:segments(block.content,block.type==='heading'),...(block.type==='details'&&block.title?.length?{title:segments(block.title)}:{})});
  }
