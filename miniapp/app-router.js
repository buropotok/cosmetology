...
function show(route,{notify=true}={}){if(!screenIds[route])throw new RangeError(`Unknown application route: ${route}`);for(const [name,id] of Object.entries(screenIds)){const element=document.getElementById(id);if(element)element.hidden=name!==route}current=route;document.body.dataset.cosmoRoute=route;window.scrollTo(0,0);if(notify)telegram.notifySelection();for(const listener of listeners)listener(route);return route}
let settingsPromise;
function loadSettings(){if(!settingsPromise)settingsPromise=import('./settings-runtime.js').then(module=>module.loadSettingsRuntime()).catch(error=>{settingsPromise=undefined;console.error('Settings runtime failed to load',error)});return settingsPromise}
async function openSettings(){if(current&&current!=='settings'&&current!=='onboarding')settingsReturn=current;const route=show('settings');await loadSettings();return route}
...
document.addEventListener('click',event=>{const target=event.target.closest?.('#open-settings,.cosmo-flow-settings,#close-settings');if(!target)return;if(target.id==='close-settings')closeSettings();else void openSettings()});
