...
document.addEventListener('click',async event=>{const button=event.target.closest?.('#back,#finish');if(!button||button.disabled||saving)return;event.preventDefault();event.stopImmediatePropagation();if(button.id==='back'){clearTimeout(saveTimer);close('back');void persistDraft().catch(()=>{});return}saving=true;button.disabled=true;showLoader();try{await save()}catch(error){resetTransient();const target=document.getElementById('error');if(target)target.textContent=error?.message||'Не удалось сохранить изображение'}},true);
