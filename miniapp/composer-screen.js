...
const icons={
 ...
 vk:'<svg class="composer-action-icon" viewBox="0 0 48 48" aria-hidden="true"><rect width="48" height="48" rx="12" fill="#0077FF"/><path fill="#fff" d="M25.6 34.2C14.7 34.2 8.5 26.7 8.2 14.2h5.5c.2 9.2 4.2 13.1 7.3 13.9V14.2h5.2v7.9c3-.3 6.2-3.9 7.3-7.9h5.2c-.8 4.9-4.4 8.5-7 10 2.6 1.2 6.8 4.4 8.4 10h-5.8c-1.2-3.8-4.1-6.7-8.1-7.1v7.1h-.6Z"/></svg>'
};
function decorate(button,icon){if(!button||button.querySelector('.composer-action-icon'))return;button.insertAdjacentHTML('afterbegin',icon)}
function decorateAll(){decorate(preview,icons.preview);decorate(publish,icons.tg);decorate(publishVk,icons.vk)}
decorateAll();
const observer=new MutationObserver(()=>queueMicrotask(decorateAll));for(const button of [preview,publish,publishVk])if(button)observer.observe(button,{childList:true});

let vkBypass=false;
window.addEventListener('click',async event=>{
 const button=event.target.closest?.('#publish-vk');if(!button||vkBypass)return;
 event.preventDefault();event.stopImmediatePropagation();
 const guard=window.CosmoSofaAccount?.requireVkDestination;if(typeof guard!=='function')return;
 const state=await guard();if(!state)return;
 vkBypass=true;button.click();vkBypass=false;
},true);
})();
