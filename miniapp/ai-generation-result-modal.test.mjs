...
test('AI wizard preserves the previous result on failure and retries the exact owned request',()=>{
  ...
  assert.match(body,/if\(controller\.signal\.aborted\)return/);
});
...
test('Composer image acquisition reports result after the owned request and does not retry cancellation',()=>{
  ...
  assert.match(composerImages,/if\(isExpectedCancellation\(error,operation\)\)\{[^}]*return\}/);
