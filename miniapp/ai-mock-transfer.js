...
const RICH_LOADER_VERSION='2026-09-03.8';
...
import(`/composer-tiptap.js?v=${encodeURIComponent(RICH_LOADER_VERSION)}`)
 ...
 .then(()=>{
   ...
   return import(`/composer-tiptap-fixes.js?v=${encodeURIComponent(RICH_LOADER_VERSION)}`)
     .then(()=>window.CosmoDiagnostics?.log?.('details-fix-loader-ok',{version:RICH_LOADER_VERSION}))
     .catch(error=>{window.CosmoDiagnostics?.log?.('details-fix-loader-error',{version:RICH_LOADER_VERSION,error:error?.message||String(error)});console.warn('Details arrow fix load failed',error)});
 })
