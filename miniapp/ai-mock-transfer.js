...
const RICH_LOADER_VERSION='2026-09-03.2';
window.CosmoDiagnostics?.log?.('rich-loader-start',{version:RICH_LOADER_VERSION,module:'/composer-rich-text.js'});
import(`/composer-rich-text.js?v=${encodeURIComponent(RICH_LOADER_VERSION)}`)
  .then(()=>import(`/composer-editor-stability.js?v=${encodeURIComponent(RICH_LOADER_VERSION)}`))
  .then(()=>window.CosmoDiagnostics?.log?.('rich-loader-ok',{version:RICH_LOADER_VERSION,apiReady:!!window.CosmoRichEditor,stability:true}))
  .catch(error=>{window.CosmoDiagnostics?.log?.('rich-loader-error',{version:RICH_LOADER_VERSION,error:error?.message||String(error)});console.warn('Rich text editor load failed',error)});
