...
.composer-editor-fullscreen-hint{position:absolute;left:50%;top:max(10px,env(safe-area-inset-top,0px));z-index:3;max-width:calc(100% - 32px);padding:8px 12px;border-radius:999px;background:rgba(28,28,30,.88);color:#fff;font-size:13px;line-height:1.25;text-align:center;pointer-events:none;opacity:0;transform:translate(-50%,-8px);transition:opacity .18s ease,transform .18s ease}.composer-editor-fullscreen-hint.is-visible{opacity:1;transform:translate(-50%,0)}
...
export function initComposerEditorKeyboardLayout({editorApi=globalThis.window?.CosmoRichEditor,root=globalThis.document?.querySelector?.('.composer-editor'),win=globalThis.window,doc=globalThis.document,viewport=globalThis.window?.visualViewport,router=globalThis.window?.CosmoRouter}={}){
  ...
  const editor=editorApi?.editor;const host=editorApi?.element;const toolbar=root.querySelector('.composer-toolbar');const footer=root.querySelector('.composer-editor-footer');const clear=root.querySelector('.composer-clear');
  ...
  const releaseStyles=acquireStyles(doc);const toolbarParent=toolbar.parentNode,toolbarNext=toolbar.nextSibling;const clearParent=clear.parentNode,clearNext=clear.nextSibling;const controls=doc.createElement('div');controls.className='composer-editor-controls';controls.setAttribute('aria-label','Управление редактором');footer.before(controls);controls.append(toolbar,clear);
  const hint=doc.createElement('div');hint.className='composer-editor-fullscreen-hint';hint.setAttribute('role','status');hint.textContent='Смахните вверх, чтобы выйти из режима редактирования текста';root.append(hint);
  const platform=String(win.Telegram?.WebApp?.platform||'');const coarsePointer=typeof win.matchMedia==='function'&&win.matchMedia('(pointer: coarse)').matches;const mobile=isMobileEditorEnvironment({platform,coarsePointer});let active=false,pullStart=null,hintTimer=null;
  const hideHint=()=>{if(hintTimer!==null){win.clearTimeout(hintTimer);hintTimer=null}hint.classList.remove('is-visible')};
  const showHint=()=>{hideHint();hint.classList.add('is-visible');hintTimer=win.setTimeout(()=>{hintTimer=null;hint.classList.remove('is-visible')},2800)};
  ...
  const onFocus=()=>activate();const onRoute=route=>{if(route!=='composer')exitFullscreen()};const onPublishMode=event=>{if(event?.detail?.mode!=='compose')exitFullscreen()};const onBeforeAfterOpen=()=>exitFullscreen();const keepClearFocus=event=>{if(active)event.preventDefault()};
  ...
  editor.on('focus',onFocus);clear.addEventListener('mousedown',keepClearFocus);host.addEventListener('touchstart',onTouchStart,{passive:true});host.addEventListener('touchmove',onTouchMove,{passive:false});host.addEventListener('touchend',clearPull);host.addEventListener('touchcancel',clearPull);win.addEventListener('resize',updateViewport);win.addEventListener('cosmo-publish-mode',onPublishMode);win.addEventListener('cosmo-before-after-open',onBeforeAfterOpen);viewport?.addEventListener?.('resize',updateViewport);viewport?.addEventListener?.('scroll',updateViewport);const unsubscribeRoute=typeof router?.subscribe==='function'?router.subscribe(onRoute):()=>{};
  const controller={update:updateViewport,exit:exitFullscreen,destroy(){deactivate();editor.off('focus',onFocus);clear.removeEventListener('mousedown',keepClearFocus);host.removeEventListener('touchstart',onTouchStart);host.removeEventListener('touchmove',onTouchMove);host.removeEventListener('touchend',clearPull);host.removeEventListener('touchcancel',clearPull);win.removeEventListener('resize',updateViewport);win.removeEventListener('cosmo-publish-mode',onPublishMode);win.removeEventListener('cosmo-before-after-open',onBeforeAfterOpen);viewport?.removeEventListener?.('resize',updateViewport);viewport?.removeEventListener?.('scroll',updateViewport);unsubscribeRoute();hint.remove();if(toolbarParent)toolbarParent.insertBefore(toolbar,toolbarNext&&toolbarNext.parentNode===toolbarParent?toolbarNext:null);if(clearParent)clearParent.insertBefore(clear,clearNext&&clearNext.parentNode===clearParent?clearNext:null);controls.remove();instances.delete(root);releaseStyles()}};
