...
function setDot(ui,ready){if(!ui.dot)return;ui.dot.hidden=!ready;ui.dot.className='status-dot ok'}
function setAction(ui,label,{hidden=false}={}){if(!ui.button)return;ui.button.hidden=hidden;ui.button.textContent=label;ui.button.className='settings-text-action';ui.button.removeAttribute('aria-label')}
