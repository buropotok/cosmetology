...
let vkSelectionPending=false,vkExternalOpened=false,vkRefreshPromise=null;
function finishVkSelection(){vkSelectionPending=false;vkExternalOpened=false;vkRefreshPromise=null;if(vkUi.button)vkUi.button.disabled=false}
function refreshVkReturn({force=false}={}){if(!vkSelectionPending||vkRefreshPromise||(!force&&(!vkExternalOpened||document.visibilityState==='hidden')))return;vkRefreshPromise=controller.refresh().catch(error=>window.CosmoTelegramGateway.create().showAlert(error instanceof Error?error.message:'Не удалось обновить группу ВКонтакте.')).finally(finishVkSelection)}
vkUi.button?.addEventListener('click',async()=>{if(vkSelectionPending)return;vkSelectionPending=true;vkExternalOpened=false;vkUi.button.disabled=true;try{await controller.connectVk();vkExternalOpened=true}catch(error){finishVkSelection();window.CosmoTelegramGateway.create().showAlert(error instanceof Error?error.message:'Не удалось открыть выбор группы ВКонтакте.')}});
tg?.onEvent?.('activated',()=>refreshVkReturn({force:true}));document.addEventListener('visibilitychange',refreshVkReturn);window.addEventListener('focus',refreshVkReturn);
})();
