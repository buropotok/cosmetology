...
style.textContent=`
.cosmo-flow-screen{--c-bg:#f2f2f7;--c-card:#fff;--c-text:#111;--c-muted:#6e6e73;--c-line:#e5e5ea;--c-blue:#2481cc;color:var(--c-text);background:var(--c-bg);margin:-22px -18px -24px;padding:calc(22px + env(safe-area-inset-top)) 16px calc(30px + env(safe-area-inset-bottom));min-height:100vh;font-family:-apple-system,BlinkMacSystemFont,"SF Pro Text","Segoe UI",sans-serif;box-sizing:border-box}.cosmo-flow-screen[hidden]{display:none!important}.cosmo-flow-nav{height:42px;display:flex;align-items:center;justify-content:center;position:relative}.cosmo-flow-nav .cosmo-logo{position:absolute;left:0;width:36px;height:36px;background:url('/assets/icons/cosmo-sofa.svg') center/contain no-repeat}.cosmo-flow-nav h1{font-size:17px;font-weight:700;margin:0}.cosmo-home-settings{position:absolute;right:0;width:36px;height:36px;border:0;background:transparent;color:var(--c-blue);padding:6px}.cosmo-home-settings svg{width:24px;height:24px}.cosmo-flow-back{position:absolute;left:0;border:0;background:transparent;color:var(--c-blue);font:600 15px/1 inherit;padding:8px 4px}.cosmo-home-hero{padding:42px 4px 28px;text-align:center}.cosmo-home-mark{width:76px;height:76px;margin:0 auto 18px;background:url('/assets/icons/cosmo-sofa.svg') center/contain no-repeat}.cosmo-home-hero h2{font-size:28px;margin:0 0 8px}.cosmo-home-hero p{color:var(--c-muted);font-size:15px;line-height:1.45;margin:0}.cosmo-flow-actions{display:grid;gap:11px}.cosmo-primary,.cosmo-secondary{width:100%;border:0;border-radius:12px;padding:15px 16px;font:700 16px/1.2 inherit}.cosmo-primary{background:var(--c-blue);color:#fff}.cosmo-secondary{background:#fff;color:var(--c-blue)}.cosmo-ai-intro{margin:18px 0 12px}.cosmo-ai-intro strong{display:block;font-size:21px;margin-bottom:5px}.cosmo-ai-intro span{color:var(--c-muted);font-size:14px}.cosmo-ai-card{background:#fff;border-radius:15px;padding:14px;margin-bottom:14px}.cosmo-ai-head{display:flex;gap:10px;align-items:center}.cosmo-ai-avatar{width:38px;height:38px;border-radius:50%;background:var(--c-blue);color:#fff;display:grid;place-items:center;font-size:19px}.cosmo-ai-head strong{display:block;font-size:16px}.cosmo-ai-head span{display:block;color:var(--c-muted);font-size:13px}.cosmo-ai-chips{display:flex;gap:8px;overflow-x:auto;margin:14px -2px 12px;padding:2px;scrollbar-width:none}.cosmo-ai-chip{white-space:nowrap;border:0;background:#f2f2f7;border-radius:999px;padding:9px 12px;font:400 13px/1 inherit;color:#444}.cosmo-ai-chip.active{background:var(--c-blue);color:#fff}.cosmo-ai-prompt{display:flex;gap:7px;background:#f2f2f7;border-radius:11px;padding:4px 5px}.cosmo-ai-prompt input{min-width:0;flex:1;border:0;outline:0;padding:8px;background:transparent;font:14px/1.2 inherit}.cosmo-ai-prompt button{width:34px;height:34px;border:0;border-radius:9px;background:var(--c-blue);color:#fff;font-size:16px}.cosmo-ai-prompt button:disabled{opacity:.55}.cosmo-manual{margin-top:10px}.cosmo-ai-result{background:#fff;border-radius:15px;overflow:hidden;margin:14px 0}.cosmo-ai-image{height:205px;background:#e9edf1}.cosmo-ai-image svg{display:block;width:100%;height:100%}.cosmo-ai-copy{padding:15px}.cosmo-ai-copy h3{font-size:17px;margin:0 0 9px}.cosmo-ai-copy p{font-size:15px;line-height:1.5;margin:0;color:#26282c}.cosmo-ai-note{font-size:12px;color:var(--c-muted);text-align:center;margin:10px 0 0}.cosmo-composer-back{position:absolute!important;left:0!important;width:36px!important;height:36px!important;border:0!important;background:transparent!important;color:var(--c-blue)!important;font-size:25px!important;padding:0!important;z-index:3}.approved-composer .composer-nav::before{display:none!important}.cosmo-draft-loading{position:absolute;inset:0;z-index:20;display:flex;align-items:center;justify-content:center;gap:10px;background:rgba(255,255,255,.88);border-radius:inherit;font:600 14px/1.2 inherit;color:#555}.cosmo-draft-loading[hidden]{display:none!important}.cosmo-draft-spinner{width:20px;height:20px;border:2px solid #cfd4d8;border-top-color:var(--c-blue);border-radius:50%;animation:cosmo-spin .8s linear infinite}@keyframes cosmo-spin{to{transform:rotate(360deg)}}
`;
document.head.append(style);

const home=document.createElement('section');home.id='home-screen';home.className='cosmo-flow-screen';home.innerHTML=`<header class="cosmo-flow-nav"><span class="cosmo-logo" aria-hidden="true"></span><h1>Cosmo Sofa</h1></header><div class="cosmo-home-hero"><div class="cosmo-home-mark" aria-hidden="true"></div><h2>Создайте публикацию</h2><p>Подготовьте новый материал или вернитесь к сохранённому черновику.</p></div><div class="cosmo-flow-actions"><button id="flow-new" class="cosmo-primary" type="button">Новый пост</button><button id="flow-continue" class="cosmo-secondary" type="button" hidden>Продолжить работу</button></div>`;
const homeNav=home.querySelector('.cosmo-flow-nav');
if(settingsButton&&homeNav){settingsButton.className='cosmo-home-settings';homeNav.append(settingsButton)}
...
home.querySelector('#flow-new').addEventListener('click',()=>router.show('ai'));continueButton.addEventListener('click',()=>router.show('composer'));ai.querySelector('#flow-ai-back').addEventListener('click',()=>router.show('home'));
...
async function sendAiMessage(){
  ...
  if(!message){aiPrompt.focus();return}
