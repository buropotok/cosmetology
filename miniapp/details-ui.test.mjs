...
test('active Tiptap details UI matches Telegram disclosure while preserving PostDocument v2 blocks',()=>{
  ...
  assert.match(editor,/detailsBlocks=\(body\?\.content\|\|\[\]\)\.map\(nodeToPostBlock\)\.filter\(Boolean\)/);
  assert.match(editor,/blocks:detailsBlocks\.length\?detailsBlocks/);
});
