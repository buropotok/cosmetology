...
import(`/composer-tiptap.js?v=${encodeURIComponent(RICH_LOADER_VERSION)}`)
  ...
  .then(()=>{
    ...
    return import(`/composer-tiptap-fixes.js?v=${encodeURIComponent(RICH_LOADER_VERSION)}`)
      ...
      .catch(error=>{
        window.CosmoDiagnostics?.log?.('details-nodeview-loader-error',{version:RICH_LOADER_VERSION,error:error?.message||String(error)});
        console.warn('Details NodeView load failed',error);
      });
  })
  .catch(error=>{
