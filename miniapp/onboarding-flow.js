...
let bypassPreview=false,bypassPublish=false,bypassVkPublish=false;
...
document.addEventListener('click',event=>{const button=event.target.closest?.('#publish-vk');if(!button)return;if(bypassVkPublish){bypassVkPublish=false;return}event.preventDefault();event.stopImmediatePropagation();guard('vk_publish').then(ready=>{if(ready){bypassVkPublish=true;button.click()}}).catch(error=>tg?.showAlert?.(error instanceof Error?error.message:'Не удалось проверить группу ВКонтакте.'))},true);
