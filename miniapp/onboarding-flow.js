...
function openSettings(){closeModal();window.CosmoRouter?.openSettings?.()}
function activatePersonalChat(state){const username=state?.managedBot?.username;if(!username)throw new Error('Личный чат ещё не создан.');closeModal();window.CosmoTelegramGateway.create().openTelegramLink(`https://t.me/${username}`)}
