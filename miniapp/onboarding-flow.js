...
async function createIntent(action){await window.CosmoSofaDraft?.flush?.(`onboarding-${action}`);return api('/api/miniapp/onboarding-intent',{method:'POST',body:{action}})}
...
installDiagnostics();window.addEventListener('focus',()=>reconcile());document.addEventListener('visibilitychange',()=>{if(document.visibilityState==='visible')reconcile()});window.addEventListener('cosmo-onboarding-result',()=>reconcile());window.CosmoOnboardingFlow=Object.freeze({guard,reconcile,diagnostics:showDiagnostics});queueMicrotask(()=>reconcile({initial:true}));
})();
