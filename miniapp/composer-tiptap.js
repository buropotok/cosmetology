...
(()=>{
  ...
  let buttons=[];
  function toPostDocument(){const json=editor.getJSON(),blocks=[];for(const node of json.content||[]){if(node.type==='paragraph'){blocks.push({type:'paragraph',content:inlineRuns(node)});continue}if(node.type==='heading'){blocks.push({type:'heading',content:inlineRuns(node)});continue}if(node.type==='blockquote'){blocks.push({type:'quote',content:inlineRuns(node)});continue}if(node.type==='orderedList'||node.type==='bulletList'){blocks.push({type:node.type==='orderedList'?'ordered_list':'bullet_list',items:(node.content||[]).filter(n=>n.type==='listItem').map(normalizeListItem)});continue}if(node.type==='details'){const summary=(node.content||[]).find(n=>n.type==='detailsSummary'),body=(node.content||[]).find(n=>n.type==='detailsBody');blocks.push({type:'details',title:summary?inlineRuns(summary):[{text:'Подробнее'}],content:body?inlineRuns(body):[]});continue}}if(!blocks.length)blocks.push({type:'paragraph',content:[{text:''}]});return{schemaVersion:1,blocks,...(buttons.length?{buttons:[...buttons]}:{})}}
  function postToTiptap(doc){const runNodes=runs=>(runs||[]).flatMap(run=>{if(!run.text)return[];const marks=(run.marks||[]).map(mark=>mark.type==='strikethrough'?{type:'strike'}:mark.type==='link'?{type:'link',attrs:{href:mark.href,target:'_blank',rel:'noopener noreferrer nofollow',class:null}}:{type:mark.type});return[{type:'text',text:run.text,marks:marks.length?marks:undefined}]});const listItem=item=>{item=Array.isArray(item)?{content:item}:item;const content=[{type:'paragraph',content:runNodes(item.content)}];if(item.children?.length)content.push({type:'bulletList',content:item.children.map(listItem)});return{type:'listItem',content}};const content=[];for(const block of doc?.blocks||[]){if(block.type==='paragraph')content.push({type:'paragraph',content:runNodes(block.content)});else if(block.type==='heading')content.push({type:'heading',attrs:{level:1},content:runNodes(block.content)});else if(block.type==='quote')content.push({type:'blockquote',content:[{type:'paragraph',content:runNodes(block.content)}]});else if(block.type==='ordered_list'||block.type==='bullet_list')content.push({type:block.type==='ordered_list'?'orderedList':'bulletList',content:(block.items||[]).map(listItem)});else if(block.type==='details')content.push({type:'details',content:[{type:'detailsSummary',content:runNodes(block.title?.length?block.title:[{text:'Подробнее'}])},{type:'detailsBody',content:[{type:'paragraph',content:runNodes(block.content)}]}]})}return{type:'doc',content:content.length?content:[{type:'paragraph'}]}}
