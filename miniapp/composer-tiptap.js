...
(()=>{
  ...
  const normalizeListItem=node=>{const first=(node.content||[]).find(n=>n.type==='paragraph');const item={content:first?inlineRuns(first):[]};const nested=(node.content||[]).find(n=>n.type==='orderedList'||n.type==='bulletList');if(nested){const children=(nested.content||[]).filter(n=>n.type==='listItem').map(normalizeListItem);if(children.length)item.children=children}return item};
  const nodeToPostBlock=node=>node.type==='paragraph'?{type:'paragraph',content:inlineRuns(node)}:node.type==='heading'?{type:'heading',content:inlineRuns(node)}:node.type==='blockquote'?{type:'quote',content:inlineRuns(node)}:node.type==='orderedList'||node.type==='bulletList'?{type:node.type==='orderedList'?'ordered_list':'bullet_list',items:(node.content||[]).filter(n=>n.type==='listItem').map(normalizeListItem)}:null;
  ...
  let buttons=[];
  function toPostDocument(){const json=editor.getJSON(),blocks=[];for(const node of json.content||[]){if(node.type==='details'){const summary=(node.content||[]).find(n=>n.type==='detailsSummary'),body=(node.content||[]).find(n=>n.type==='detailsBody'),detailsBlocks=(body?.content||[]).map(nodeToPostBlock).filter(Boolean);blocks.push({type:'details',title:summary?inlineRuns(summary):[{text:'Подробнее'}],blocks:detailsBlocks.length?detailsBlocks:[{type:'paragraph',content:[]}]});continue}const block=nodeToPostBlock(node);if(block)blocks.push(block)}if(!blocks.length)blocks.push({type:'paragraph',content:[{text:''}]});return{schemaVersion:2,blocks,...(buttons.length?{buttons:[...buttons]}:{})}}
  function postToTiptap(doc){const runNodes=runs=>(runs||[]).flatMap(run=>{if(!run.text)return[];const marks=(run.marks||[]).map(mark=>mark.type==='strikethrough'?{type:'strike'}:mark.type==='link'?{type:'link',attrs:{href:mark.href,target:'_blank',rel:'noopener noreferrer nofollow',class:null}}:{type:mark.type});return[{type:'text',text:run.text,marks:marks.length?marks:undefined}]});const listItem=item=>{item=Array.isArray(item)?{content:item}:item;const content=[{type:'paragraph',content:runNodes(item.content)}];if(item.children?.length)content.push({type:'bulletList',content:item.children.map(listItem)});return{type:'listItem',content}};const blockNode=block=>block.type==='paragraph'?{type:'paragraph',content:runNodes(block.content)}:block.type==='heading'?{type:'heading',attrs:{level:1},content:runNodes(block.content)}:block.type==='quote'?{type:'blockquote',content:[{type:'paragraph',content:runNodes(block.content)}]}:block.type==='ordered_list'||block.type==='bullet_list'?{type:block.type==='ordered_list'?'orderedList':'bulletList',content:(block.items||[]).map(listItem)}:null;const content=[];for(const block of doc?.blocks||[]){if(block.type==='details'){const legacyBlocks=Array.isArray(block.blocks)?block.blocks:[{type:'paragraph',content:block.content||[]}],body=legacyBlocks.map(blockNode).filter(Boolean);content.push({type:'details',content:[{type:'detailsSummary',content:runNodes(block.title?.length?block.title:[{text:'Подробнее'}])},{type:'detailsBody',content:body.length?body:[{type:'paragraph'}]}]});continue}const node=blockNode(block);if(node)content.push(node)}return{type:'doc',content:content.length?content:[{type:'paragraph'}]}}
