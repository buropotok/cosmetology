...
(()=>{
  ...
  style.textContent=`
    ...
    .composer-emoji-panel{position:fixed;z-index:130;display:flex;flex-wrap:wrap;gap:4px;width:220px;box-sizing:border-box;padding:6px 8px 8px;background:#fff;border:1px solid #ddd;border-radius:12px;box-shadow:0 8px 30px #0002;touch-action:none;user-select:none;-webkit-user-select:none}
    .composer-emoji-drag-handle{flex:0 0 100%;height:18px;display:grid;place-items:center;cursor:grab;touch-action:none}
    .composer-emoji-drag-handle::before{content:'';width:34px;height:4px;border-radius:999px;background:#c9c9ce}
  ...
  function visualBounds(){const viewport=window.visualViewport;return{left:Number(viewport?.offsetLeft)||0,top:Number(viewport?.offsetTop)||0,width:Number(viewport?.width)||innerWidth,height:Number(viewport?.height)||innerHeight}}
  function placeEmojiPanel(panel,left,top){const bounds=visualBounds(),rect=panel.getBoundingClientRect(),margin=8,maxLeft=Math.max(bounds.left+margin,bounds.left+bounds.width-rect.width-margin),maxTop=Math.max(bounds.top+margin,bounds.top+bounds.height-rect.height-margin);panel.style.left=`${Math.max(bounds.left+margin,Math.min(maxLeft,left))}px`;panel.style.top=`${Math.max(bounds.top+margin,Math.min(maxTop,top))}px`}
  function positionEmojiPanel(panel,anchor){const anchorRect=anchor.getBoundingClientRect(),panelRect=panel.getBoundingClientRect();placeEmojiPanel(panel,anchorRect.right-panelRect.width,anchorRect.top-panelRect.height-8)}
  ...
  const emojiBtn=toolbar.querySelector('[title="Emoji"]');if(emojiBtn){emojiBtn.addEventListener('mousedown',preserve);emojiBtn.addEventListener('click',e=>{e.preventDefault();document.querySelector('.composer-emoji-panel')?.remove();const panel=document.createElement('div');panel.className='composer-emoji-panel';panel.style.visibility='hidden';const handle=document.createElement('div');handle.className='composer-emoji-drag-handle';handle.setAttribute('aria-label','Переместить панель эмодзи');panel.append(handle);for(const emoji of ['😀','😊','😍','🥰','✨','💫','🌿','🌸','💧','🧴','💆‍♀️','❤️','🤍','👍','🔥','📌','✅','⚠️','💡','👉']){const b=document.createElement('button');b.type='button';b.textContent=emoji;b.addEventListener('mousedown',preserve);b.addEventListener('click',()=>{editor.chain().focus().insertContent(emoji).run();panel.remove()});panel.append(b)}document.body.append(panel);positionEmojiPanel(panel,emojiBtn);makeEmojiPanelDraggable(panel,handle);panel.style.visibility=''})}
