...
(()=>{
  ...
  style.textContent=`
    ...
    .composer-emoji-scroll button{width:38px;height:38px;border:0;background:transparent;font-size:24px;padding:4px;border-radius:8px}
    .composer-emoji-scroll button:active{background:#0000000d}
  ...
  const Details=Node.create({name:'details',priority:1000,group:'block',content:'detailsSummary detailsBody',isolating:true,parseHTML(){return[{tag:'details'}]},renderHTML({HTMLAttributes}){return['details',mergeAttributes(HTMLAttributes),0]},addKeyboardShortcuts(){return{Backspace:()=>{const {selection}=this.editor.state,selected=selectedWholeDetails(selection);if(selected)return removeDetails(this.editor,selected);if(!selection.empty)return false;const current=detailsAtResolvedPos(selection.$from);return current&&current.node.child(1).textContent.length===0?removeDetails(this.editor,{from:current.from,to:current.to}):false}}}});
