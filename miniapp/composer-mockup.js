(()=>{
  ...
  const photoCard=document.createElement('section');photoCard.className='composer-card composer-photo-card';photoCard.innerHTML=`<div class="composer-photo-edit-hint" hidden>Нажмите на фото для редактирования</div><div class="composer-image"><div><svg class="composer-photo-icon" viewBox="0 0 48 48" aria-hidden="true"><rect x="7" y="10" width="34" height="28" rx="5" fill="none" stroke="#555b64" stroke-width="2.2"/><circle cx="18" cy="20" r="3.5" fill="none" stroke="#555b64" stroke-width="2.2"/><path d="M10 34l9-9 7 7 5-5 7 7" fill="none" stroke="#555b64" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"/></svg><b>Фото для поста</b><small>До 10 изображений для Telegram и VK</small></div></div><div class="composer-image-actions"><button type="button" id="composer-add-photo">＋ Добавить фото</button></div>`;
  form.before(photoCard);photoCard.after(imageInput);
  const photoStage=photoCard.querySelector('.composer-image'),photoEditHint=photoCard.querySelector('.composer-photo-edit-hint');
  const syncPhotoEditHint=()=>{photoEditHint.hidden=!(imageInput.files?.length||0)};
  photoCard.querySelector('#composer-add-photo')?.addEventListener('click',()=>imageInput.click());
  photoStage?.addEventListener('click',()=>{if(!(imageInput.files?.length||0))imageInput.click()});
  imageInput.addEventListener('change',syncPhotoEditHint);
  syncPhotoEditHint();
