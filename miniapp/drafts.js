...
let beforeAfterFiles=new Map(),beforeAfterFilesInitialized=false;
...
async function load(){const result=await store.load();beforeAfterFilesInitialized=false;return result}
async function whenReady(){const current=store.getState();if(current.loadStatus==='ready')return current;await load();return store.getState()}
async function clear(){const result=await store.clear();beforeAfterFiles.clear();beforeAfterFilesInitialized=true;return result}
...
window.CosmoSofaDraft=Object.freeze({load,whenReady,scheduleSave:store.scheduleSave,flush:store.flush,save:()=>store.flush('api-save'),clear,cancelRestore:store.cancelRestore,getState:store.getState,setScreen:store.setScreen,setBeforeAfterState:store.setBeforeAfterState,setBeforeAfterImage,swapBeforeAfterImages,getBeforeAfterDraft});
