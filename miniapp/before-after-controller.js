(()=>{
  ...
  function close(){if(!overlay)return;frameBridge()?.resetTransient?.();if(currentMode==='solo'){overlay.remove();overlay=null;soloIndex=null;soloFile=null;currentMode='dual'}else overlay.hidden=true;document.documentElement.style.overflow='';window.scrollTo({top:0,behavior:'instant'})}
