(()=>{
  ...
  async function saveDraft(snapshot){if(currentMode!=='dual'||!snapshot?.state)return false;const body=baseDraftBody(),stateJson=JSON.stringify(snapshot.state);body.set('beforeAfterState',stateJson);let response,result;try{response=await fetch('/api/miniapp/draft',{method:'POST',headers:authHeaders(),body});const raw=await response.text();try{result=raw?JSON.parse(raw):null}catch{result={raw}}}catch(error){throw error}if(!response.ok)throw new Error(result?.error?.message||'Не удалось сохранить черновик До/После.');window.CosmoSofaDraft?.setBeforeAfterState?.(snapshot.state,{persist:false});window.CosmoSofaDraft?.setScreen?.('beforeafter',{persist:false});return true}
  async function saveAsset(role,file){const body=new FormData();body.set('role',role);body.set('image',file,file.name||`${role}.jpg`);const response=await fetch('/api/miniapp/before-after/asset',{method:'POST',headers:authHeaders(),body}),result=await response.json().catch(()=>null);if(!response.ok)throw new Error(result?.error?.message||'Не удалось сохранить фото До/После.');window.CosmoSofaDraft?.setBeforeAfterImage?.(role,file);return result}
  async function removeAsset(role){const response=await fetch('/api/miniapp/before-after/remove',{method:'POST',headers:{...authHeaders(),'content-type':'application/json'},body:JSON.stringify({role})}),result=await response.json().catch(()=>null);if(!response.ok)throw new Error(result?.error?.message||'Не удалось удалить фото из черновика.');window.CosmoSofaDraft?.setBeforeAfterImage?.(role,null);return result}
  async function swapAssets(){const response=await fetch('/api/miniapp/before-after/swap',{method:'POST',headers:authHeaders()}),result=await response.json().catch(()=>null);if(!response.ok)throw new Error(result?.error?.message||'Не удалось поменять фото местами.');window.CosmoSofaDraft?.swapBeforeAfterImages?.();return result}
