(()=>{
  ...
  function close(){if(!overlay)return;frameBridge()?.resetTransient?.();if(currentMode==='solo')frameBridge()?.destroySoloSession?.();if(currentMode==='solo')destroySolo();else overlay.hidden=true;document.documentElement.style.overflow='';window.scrollTo({top:0,behavior:'instant'})}
  function clear(){if(!overlay)return;if(currentMode==='solo')frameBridge()?.destroySoloSession?.();overlay.remove();overlay=null;soloIndex=null;soloFile=null;currentMode='dual';document.documentElement.style.overflow=''}
  ...
  async function saveDraft(snapshot){if(currentMode!=='dual'||!snapshot?.state)return false;const draft=window.CosmoSofaDraft;if(!draft?.setBeforeAfterState||!draft?.flush)throw new Error('Черновик недоступен.');debugLog('STATE SAVE requested',{state:snapshot.state});await draft.setBeforeAfterState(snapshot.state,{persist:true});const saved=await draft.flush('before-after-state');debugLog('STATE SAVE completed',{saved});if(saved!==true)throw new Error('Не удалось сохранить черновик До/После.');return true}
