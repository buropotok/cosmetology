...
- `POST /api/miniapp/telegram/managed-bot/group-link` создаёт tenant-scoped одноразовую `startgroup` ссылку персонального бота; отдельный authenticated webhook `/api/telegram/managed/<opaque-id>` подтверждает группу и сохраняет managed-bot destination.
