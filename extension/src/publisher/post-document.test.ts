...
describe('platform renderers',()=>{
  ...
  it('keeps schema-v2 nested bodies and marks in the preview block contract',()=>{const rendered=renderTelegram({schemaVersion:2,blocks:[{type:'details',title:[{text:'More'}],blocks:[{type:'paragraph',content:[{text:'Bold',marks:[{type:'bold'}]}]},{type:'bullet_list',items:[[{text:' item',marks:[{type:'italic'}]}]]}]},{type:'quote',blocks:[{type:'paragraph',content:[{text:'Link',marks:[{type:'link',href:'https://example.com'}]}]}]}]});const details=rendered.blocks[0],quote=rendered.blocks[1];expect(details.segments.map(segment=>segment.text).join('')).toBe('Bold\n•  item');expect(details.segments.find(segment=>segment.text==='Bold')?.marks).toEqual([{type:'bold'}]);expect(details.segments.find(segment=>segment.text===' item')?.marks).toEqual([{type:'italic'}]);expect(quote.segments).toEqual([{text:'Link',marks:[{type:'link',href:'https://example.com'}]}])});
});
