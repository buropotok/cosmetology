...
describe('PostEditor formatting toggles',()=>{
  ...
  it('creates the next numbered item on Enter',()=>{const editor=new PostEditor(document.querySelector('#editor')!,undefined,'Первый',()=>{}),block=document.querySelector<HTMLElement>('[data-block]')!,range=document.createRange();range.selectNodeContents(block);getSelection()!.removeAllRanges();getSelection()!.addRange(range);editor.block('ordered_list');document.querySelector('#editor')!.dispatchEvent(new InputEvent('beforeinput',{inputType:'insertParagraph',bubbles:true,cancelable:true}));expect((editor.document().blocks[0] as any).items).toEqual([[{text:'Первый'}],[{text:''}]]);expect(document.querySelector('[data-list-item="1"]')?.getAttribute('data-prefix')).toBe('2.')});
  it.each(['ordered_list','bullet_list'] as const)('removes %s when the same control is pressed again',type=>{const editor=new PostEditor(document.querySelector('#editor')!,undefined,'Один\nДва',()=>{}),blocks=document.querySelectorAll<HTMLElement>('[data-block]'),range=document.createRange();range.setStart(blocks[0].firstChild!,0);range.setEnd(blocks[1].firstChild!,3);getSelection()!.removeAllRanges();getSelection()!.addRange(range);expect(editor.block(type)).toBe(true);expect(editor.block(type)).toBe(false);expect(editor.document().blocks).toEqual([{type:'paragraph',content:[{text:'Один'}]},{type:'paragraph',content:[{text:'Два'}]}])});
