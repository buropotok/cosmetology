  ...
  if (typeof value === "object") {
    ...
    for (const [key, item] of Object.entries(value)) {
      if (/token/i.test(key)) out[key] = "***";
        ...
        try { const url = new URL(String(item)); out[key] = url.origin + url.pathname; }
        catch { out[key] = String(item); }
...
async function bridge(name, payload) {
  ...
  try { const result = await vkBridge.send(name, payload); log(`← Bridge ${name}`, result); return result; }
  catch (error) { log(`← Bridge ${name} ERROR`, error); throw error; }
}
async function callVkApi(method, params) {
  log(`→ VK API ${method}`, params);
  try {
    const result = await bridge("VKWebAppCallAPIMethod", { method, params: { ...params, v: VK_API_VERSION } });
    if (result?.response === undefined) throw new Error("VK API не вернул результат");
    return result.response;
  } catch (error) { throw new Error(vkError(method, error)); }
}
...
async function loadArtifact() {
  ...
  const endpoint = currentToken ? `/api/artifacts/${encodeURIComponent(currentToken)}` : "/api/test-artifact";
  log(`→ GET ${endpoint}`, { mode: currentToken ? "replicated-artifact" : "test-artifact" });
  let result = await fetchArtifact(endpoint);
  if (currentToken && result.response.status === 409 && result.body?.error?.code === "ARTIFACT_NOT_READY") {
  ...
  if (!result.response.ok) throw new Error(result.body?.error?.message || `Artifact API returned ${result.response.status}`);
  artifact = result.body;
  textEl.textContent = artifact.text || "";
  imagesEl.replaceChildren();
  for (const imageUrl of artifact.images || []) {
    const image = document.createElement("img"); image.src = imageUrl; image.alt = "Изображение публикации"; image.loading = "eager"; imagesEl.appendChild(image);
  }
...
async function uploadPhotoAttempt(auth, index, attempt, imageCount) {
  ...
  const server = await callVkApi("photos.getWallUploadServer", { access_token: auth.access_token, group_id: artifact.vkGroupId });
  if (!server?.upload_url) throw new Error("photos.getWallUploadServer: нет upload_url");
  let host = ""; try { host = new URL(server.upload_url).hostname; } catch {}
  log(`Фото ${index + 1}: upload server`, { attempt, host, uploadUrl: server.upload_url });
  const response = await fetch(`/api/artifacts/${encodeURIComponent(currentToken)}/vk-upload`, {
    method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ uploadUrl: server.upload_url, index })
  });
  const raw = await response.text();
  log(`← Yandex upload фото ${index + 1}`, { attempt, status: response.status, body: raw });
  let uploaded = null; try { uploaded = raw ? JSON.parse(raw) : null; } catch {}
  if (!response.ok) {
    const error = new Error(uploaded?.error?.message || `HTTP ${response.status}`);
    error.retryable = response.status >= 500 || uploaded?.error?.code === "VK_IMAGE_UPLOAD_INVALID" || uploaded?.error?.code === "VK_IMAGE_UPLOAD_FAILED";
    error.raw = raw; throw error;
  }
  if (!uploaded?.photo || uploaded?.server === undefined || !uploaded?.hash) { const error = new Error("неполный ответ VK upload"); error.retryable = true; error.raw = raw; throw error; }
  return uploaded;
}
async function preparePhotoAttachment(auth, index, imageCount) {
  let uploaded;
  for (let attempt = 1; attempt <= 2; attempt++) {
    try { uploaded = await uploadPhotoAttempt(auth, index, attempt, imageCount); break; }
    catch (error) {
      log(`Фото ${index + 1}: попытка ${attempt} FAILED`, { message: error instanceof Error ? error.message : String(error), retryable: error?.retryable !== false });
      if (attempt === 2 || error?.retryable === false) throw error;
      log(`Фото ${index + 1}: повторяем с новым upload server`);
    }
  }
  const saved = await callVkApi("photos.saveWallPhoto", { access_token: auth.access_token, group_id: artifact.vkGroupId, photo: uploaded.photo, server: uploaded.server, hash: uploaded.hash });
  const photo = Array.isArray(saved) ? saved[0] : null;
  if (!photo?.owner_id || !photo?.id) throw new Error(`Фото ${index + 1}: photos.saveWallPhoto не вернул фотографию`);
...
async function init() {
  try {
    log("Mini App init", { href: location.href, appId: VK_APP_ID, hasHandoff: Boolean(handoffToken()), platform: new URLSearchParams(location.search).get("vk_platform") || null });
    if (!window.vkBridge) throw new Error("VK Bridge недоступен");
    await bridge("VKWebAppInit");
    log("Bridge support", { showWallPostBox: typeof vkBridge.supports === "function" ? vkBridge.supports("VKWebAppShowWallPostBox") : "supports() unavailable", getClientVersion: typeof vkBridge.supports === "function" ? vkBridge.supports("VKWebAppGetClientVersion") : "supports() unavailable" });
    try { const client = await bridge("VKWebAppGetClientVersion"); log("VK client diagnostic", client); }
    catch (error) { log("VK client diagnostic unavailable", { message: vkError("VKWebAppGetClientVersion", error) }); }
    await loadArtifact();
  } catch (error) { statusEl.textContent = "Не удалось подготовить публикацию"; log("INIT ERROR", { message: error?.message || String(error) }); diagnosticsEl.open = true; }
}
init();
